...
- Performance: every image/video carries explicit intrinsic `width`/`height` (to avoid layout shift), below-the-fold images are `loading="lazy"` and `decoding="async"`, and the LCP hero/headshot images are eager with `fetchpriority="high"`. The whole site uses a thin, translucent scrollbar.
