...
- Capitalization pass: UI labels switched from all-lowercase to Title Case (nav links, hero eyebrow, hero/footer links, section labels, category tags, the "View Work" cue, résumé button, footer copyright). Done by setting Title Case text in the markup and flipping `text-transform: lowercase` to `none` on those classes. Figure captions, project metadata lines, core-skills lines, and date ranges stay lowercase. CLAUDE.md typography rule and label references synced. Stylesheet at `styles.css?v=21`.
